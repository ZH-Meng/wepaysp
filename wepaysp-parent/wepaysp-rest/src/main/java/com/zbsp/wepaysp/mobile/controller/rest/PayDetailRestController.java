...
import com.zbsp.wepaysp.mobile.model.paydetail.v1_0.QueryPayDetailRequest;
import com.zbsp.wepaysp.mobile.model.paydetail.v1_0.QueryPayDetailResponse;
import com.zbsp.wepaysp.mobile.result.CommonResult;

@RestController("/paydetail/v1")
public class PayDetailRestController extends BaseController {
    ...
    public QueryPayDetailResponse query(@RequestBody QueryPayDetailRequest request) {
        ...
        } else if (request.getQueryType() == QueryPayDetailRequest.QueryType.bill.getValue() && (StringUtils.isBlank(request.getBeginTime()) || StringUtils.isBlank(request.getEndTime()))) {
        	response = new QueryPayDetailResponse(CommonResult.ARGUMENT_MISS.getCode(), CommonResult.ARGUMENT_MISS.getDesc(), responseId);
        } else {
            try {
                response = new QueryPayDetailResponse(CommonResult.SUCCESS.getCode(), "处理成功", responseId);
                ...
                //Map<String, String> resultMap = weixinPayDetailsService.doJoinTransQueryWeixinPayDetails(request.getDealerEmployeeOid(), paramMap, request.getPageNum());
